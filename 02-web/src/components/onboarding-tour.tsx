...
export function OnboardingTour({ steps, onDone }: { steps: TourStep[]; onDone: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100]">
      ...
      {rect ? (
        <div
          className="fixed rounded-xl ring-2 ring-indigo-400 transition-all duration-200 pointer-events-none"
          style={{
      ...
      <div style={cardStyle} className="rounded-2xl border bg-background shadow-xl p-4">
        <div className="flex items-start justify-between gap-2">
          ...
          <button onClick={onDone} className="text-muted-foreground hover:text-foreground -mt-0.5" title={t("onboarding.skipTour")}><X className="w-4 h-4" /></button>
        </div>
        <p className="text-sm text-muted-foreground mt-1.5 leading-relaxed">{step.body}</p>
        <div className="flex items-center justify-between mt-4">
          <div className="flex gap-1">
            {steps.map((_, idx) => (
              <span key={idx} className={idx === i ? "w-4 h-1.5 rounded-full bg-indigo-700" : "w-1.5 h-1.5 rounded-full bg-muted-foreground/30"} />
            ))}
          </div>
          <div className="flex items-center gap-2">
            <button onClick={onDone} className="text-xs text-muted-foreground hover:text-foreground">{t("onboarding.skip")}</button>
            <Button size="sm" className="h-7 text-xs bg-indigo-700 hover:bg-indigo-800 text-white" onClick={() => (isLast ? onDone() : setI(i + 1))}>
