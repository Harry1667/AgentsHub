...
- **子域名清單**（2026-05-21 快照）:
  - Node 服務：`mentora` / `reportassist` / `chatcal` / `imageanalysis` / `socialbot` / `documentchatbot`
  - 靜態 / PHP：`survivalwallet` / `homeworksolver` / `keystorage` / `linkchannel` / `nearsafe` / `worklog` / `zhijian` / `mathbox`
  - 全部 `*.looptw.com` -> 伺服器入口 `137.131.7.230`
...
> ⚠️ **此表為 2026-05-21 快照，不要當權威來源。** 每次部署前用下方掃描指令抓即時狀態。

| 域名 (Domain) | 類型 | 內部 Port | 進程名稱 (PM2) | 狀態 |
| :--- | :--- | :--- | :--- | :--- |
...
| `survivalwallet` 等靜態/PHP | SPA / PHP | N/A | — | PHP-FPM / 純前端 |

- `3003` = docker-proxy；`3004`、`3010` 已被內部服務佔用。
- **下一個 Node 專案 Port：3009**（仍以掃描為準）。
- ⚠️ 程式若用 `process.env.PORT || 3001` fallback，務必在 PM2 設 `PORT` env 後 `pm2 save`，否則撞 port crash loop（mathbox 曾 restart 46 萬次）。

### 即時掃描指令（部署前必跑）

```bash
# sudo su 後執行，列出 nginx 路由 → 實際監聽程序 → 已佔用 port
bash -c '
printf "%-28s %-8s %-22s %s\n" 域名 nginxPort 監聽程序 狀態
for f in /www/server/panel/vhost/nginx/*.looptw.com.conf; do
  [ -e "$f" ] || continue; d=$(basename "$f" .conf)
  ...
  printf "%-28s %-8s %-22s %s\n" "$d" "$p" "${proc:--}" "$([ -n "$proc" ] && echo OK || echo ⚠️無監聽)"
done
echo "佔用中 30xx:"; ss -tlnp 2>/dev/null | grep -oE "[*:.]30[0-9][0-9] " | grep -oE "30[0-9]+" | sort -u'
```

## 4. AI 部署自動化指令集 (Agent Deployment SOP)

### A. 若要新增 Node.js 專案
1. **Port 分配**: 先跑上方「即時掃描指令」，挑佔用清單外最小的 port（目前約 **3008**）。
